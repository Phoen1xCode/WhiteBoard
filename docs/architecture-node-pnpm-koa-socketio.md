...
```txt
fcca376 chore: add .worktrees/ to .gitignore for git worktree isolation
```

该提交位于 Bun 引入之前，因此当前基线具备以下特点：

- 仍然是 Node.js 运行时。
- 仍然是 Koa + Socket.IO 技术路线。
- 仍然保留旧版前后端协作白板功能。
- 尚未引入 Bun workspace、Bun runtime、Bun 原生 WebSocket 或 Elysia。
- 尚未具备后续新增的认证、限流、Repository、OperationService、BatchWriter、compaction 等能力。

因此后续重构策略不是“从当前最新代码删回去”，而是“从干净的 Node/Koa/Socket.IO 基线逐步加回新后端领域能力”。具体执行计划见：
